...
import { Loader2, Music2, PlayCircle, ExternalLink } from "lucide-react";
...
export const TopTracksList: React.FC<TopTracksListProps> = ({ tracks, isLoading, error, onRetry }) => {
  if (isLoading) {
    return (
  ...
  if (tracks.length === 0) {
    return (
      <div className="flex items-center gap-2 text-sm text-zinc-400">
        ...
        Your Spotify top tracks will appear here after you connect.
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-3xl rounded-2xl bg-zinc-900/60 p-4 backdrop-blur">
      <h3 className="mb-3 flex items-center gap-2 text-lg font-semibold text-white">
        <PlayCircle className="h-5 w-5 text-[#1DB954]" />
        ...
        {tracks.map((track, index) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-sm text-zinc-400">{track.artistNames.join(", ")}</p>
            </div>
            <a
